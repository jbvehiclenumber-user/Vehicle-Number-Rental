...
class CompanyRepository {
    ...
    async findByPhone(phone) {
        // 정확한 매칭 시도
        const exactMatch = await prisma_1.prisma.company.findFirst({
            where: { phone },
        });
        if (exactMatch)
            return exactMatch;
        ...
        if (formattedPhone !== phone) {
            const formattedMatch = await prisma_1.prisma.company.findFirst({
        ...
        const matchedCompany = allCompanies.find((company) => this.normalizePhone(company.phone) === normalizedInput);
