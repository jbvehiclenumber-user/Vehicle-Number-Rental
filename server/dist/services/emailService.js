...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background-color: #f8f9fa; padding: 30px; border-radius: 8px;">
    ...
    `.trim();
        ...
        if (error) {
            logger_1.logger.error("Failed to send password reset email", new Error(error.message || "Unknown error"), {
                email,
        ...
        logger_1.logger.error("Error sending password reset email", error instanceof Error ? error : new Error(String(error)), {
            email,
        });
        throw error;
    }
