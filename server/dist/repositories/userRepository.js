...
class UserRepository {
    ...
    async findByPhone(phone) {
        // 정확한 매칭 시도
        const exactMatch = await prisma_1.prisma.user.findUnique({
            where: { phone },
        });
        if (exactMatch)
            return exactMatch;
        ...
        if (formattedPhone !== phone) {
            const formattedMatch = await prisma_1.prisma.user.findUnique({
        ...
        const matchedUser = allUsers.find((user) => this.normalizePhone(user.phone) === normalizedInput);
